...
# cloudmonkey.org/hyper-ops-bundle:$VERSION and cloudmonkey.org/hyper-ops-catalog:$VERSION.
IMAGE_TAG_BASE ?= quay.io/cldmnky/hyper-ops

# CATALOG_VERSION defines the project version for the catalog.
...
# ENVTEST_VERSION is the version of the setup-envtest tool (and the controller-runtime release it belongs to).
...
build-multiarch: gox generate fmt vet ## Build zupd binary.
	${GOX} -osarch=${RELEASE_IMAGE_PLATFORMS} -output="bin/release/{{.OS}}/{{.Arch}}/hyper-ops" github.com/cldmnky/hyper-ops/cmd
...
.PHONY: docker-build
docker-build: build-multiarch ## Build docker image with the manager.
	@cp Dockerfile bin/release/Dockerfile
	@$(CONTAINER_TOOL) build --manifest ${IMG} --pull --platform linux/amd64,linux/arm64 bin/release
...
CATALOG_IMG ?= $(IMAGE_TAG_BASE)-catalog:${CATALOG_VERSION}

# Set CATALOG_BASE_IMG to an existing catalog image tag to add $BUNDLE_IMGS to that image.
ifneq ($(origin CATALOG_IMG), undefined)
FROM_INDEX_OPT := --from-index quay.io/cldmnky/hyper-ops-catalog:v1.0
endif
...
catalog-build: opm ## Build a catalog image.
	$(OPM) index add --container-tool $(CONTAINER_TOOL) --mode semver --tag $(CATALOG_IMG) --bundles $(BUNDLE_IMGS) $(FROM_INDEX_OPT) --generate --permissive
	@$(CONTAINER_TOOL) rmi $(CATALOG_IMG)
	$(CONTAINER_TOOL) build --manifest $(CATALOG_IMG) --platform linux/amd64,linux/arm64 -f index.Dockerfile

# Push the catalog image.
.PHONY: catalog-push
catalog-push: ## Push a catalog image.
	@$(CONTAINER_TOOL) manifest push $(CATALOG_IMG) docker://$(CATALOG_IMG)
